...
`,Tn=new Set(["wav","mp3","m4a","aac","ogg","flac","wma","opus"]),vt=4,Nn=1.5,On=.25,Qe="voicepicker.json";let N=[],j=-1,ke=null,H="single",U=0,V=null,bt,Pe,ze=null,te=Ln(),P=Dn(),O=Pn(),et=0,ae=!0,le=[],z="text",fe=null,G=null,Ae=null,B=[],W=0,xe=null,Me="rating",ue=!1,q="all",oe=0;const w=i=>document.getElementById(i),yt=w("pick"),Et=w("exportBtn"),xt=w("importBtn"),Te=w("importFile"),Ct=w("editChars"),wt=w("roleViewBtn"),kt=w("filelist"),Gt=w("nowplaying"),tt=w("status"),Ie=w("playBtn"),_n=w("appVer"),zn=w("sidebar"),An=w("player"),Ne=w("grid"),$=w("roleview"),In=w("comments"),Bn=w("commentsHead"),Oe=w("commentlist"),nt=w("composer"),pe=w("composerToggle"),ge=w("composerText"),Yt=w("composerChars"),Lt=w("composerRatingEl"),$e=w("nameModal"),Be=w("nameInput"),it=w("charModal"),St=w("charEditList"),Ke=w("newCharName"),Wn=w("newCharRole"),Zt=w("addCharBtn"),Hn=w("closeCharBtn"),Rt=w("ratingsBtn"),se=w("shareBtn"),D=ye.create({container:"#waveform",waveColor:"#7a8ba6",progressColor:"#3b82f6",cursorColor:"#ef4444",height:120}),qe=D.registerPlugin(Ye.create());D.on("ready",()=>{tt.textContent=`${We(0)} / ${We(D.getDuration())}`});D.on("timeupdate",i=>{tt.textContent=`${We(i)} / ${We(D.getDuration())}`});D.on("play",()=>{Ie.textContent="⏸ 暫停",H==="role"&&Ce()});D.on("pause",()=>{Ie.textContent="▶ 播放",H==="role"&&Ce()});D.on("finish",()=>{H==="single"&&j<N.length-1?ne(j+1):H==="role"&&(xe=null,Ce())});qe.on("region-clicked",(i,e)=>{e.stopPropagation(),D.setTime(i.start),D.play()});Ie.addEventListener("click",()=>{Ie.blur(),D.playPause()});function rt(i){if(!isFinite(i))return"0:00";const e=Math.floor(i/60),t=Math.floor(i%60).toString().padStart(2,"0");return`${e}:${t}`}function We(i){if(!isFinite(i))return"0:00.00";const e=Math.floor(i/60),t=Math.floor(i%60),n=Math.floor((i-Math.floor(i))*100);return`${e}:${t.toString().padStart(2,"0")}.${n.toString().padStart(2,"0")}`}function Qt(){return Math.random().toString(36).slice(2)+Date.now().toString(36)}function Je(i){return i.code==="Enter"||i.code==="NumpadEnter"}function st(i,e){const t=i.indexOf(e);t>=0?i.splice(t,1):i.push(e)}function me(i){return i.replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function Fe(){return j>=0&&j<N.length?N[j].name:null}function $n(){return H==="grid"?U>=0&&U<N.length?N[U].name:null:Fe()}function ot(i){return i?P.filter(e=>e.file===i).sort((e,t)=>(e.time??-1/0)-(t.time??-1/0)):[]}function en(i){return P.filter(e=>e.file===i).length}function Fn(i){return(i.length?Xt(i[0],O):null)??"#3b82f6"}function Z(){tn(),je(),Ue(),de(),H==="role"&&ce()}function Q(){var i;de(),H==="role"&&ce(),G&&((i=(H==="role"?$:Oe).querySelector(".creply-input input"))==null||i.focus())}function tn(){Ze(P),V&&(window.clearTimeout(bt),bt=window.setTimeout(()=>void Un(),300))}async function Un(){if(V)try{const e=await(await V.getFileHandle(Qe,{create:!0})).createWritable();await e.write(JSON.stringify({version:1,updated:new Date().toISOString(),comments:P},null,2)),await e.close()}catch{}}function nn(i){const e=new Map(P.map(t=>[t.id,t]));i.forEach(t=>e.set(t.id,t)),P=[...e.values()]}async function jn(){if(V)try{const e=await(await V.getFileHandle(Qe)).getFile(),t=JSON.parse(await e.text());Array.isArray(t==null?void 0:t.comments)&&(nn(t.comments.map(He)),Ze(P))}catch{}}function Vn(){const i=JSON.stringify({version:1,exported:new Date().toISOString(),comments:P,characters:O},null,2),e=new Blob([i],{type:"application/json"}),t=URL.createObjectURL(e),n=new Date,r=l=>String(l).padStart(2,"0"),s=`${n.getFullYear()}${r(n.getMonth()+1)}${r(n.getDate())}-${r(n.getHours())}${r(n.getMinutes())}`,o=document.createElement("a");o.href=t,o.download=`voicepicker-backup-${s}.json`,o.click(),URL.revokeObjectURL(t)}async function Xn(i){try{const e=JSON.parse(await i.text());Array.isArray(e==null?void 0:e.comments)&&nn(e.comments.map(He)),Array.isArray(e==null?void 0:e.characters)&&e.characters.length>0&&(O=e.characters,_e(O)),Z(),alert(`匯入完成：目前共 ${P.length} 則留言。`)}catch{alert("匯入失敗：檔案不是有效的 VoicePicker 備份。")}}Et.addEventListener("click",()=>{Et.blur(),Vn()});xt.addEventListener("click",()=>{xt.blur(),Te.click()});Te.addEventListener("change",()=>{var e;const i=(e=Te.files)==null?void 0:e[0];i&&Xn(i),Te.value=""});yt.addEventListener("click",async()=>{var e;yt.blur();const i=window.showDirectoryPicker;if(!i){alert("此瀏覽器不支援資料夾選取，請改用 Chrome / Edge。");return}try{V=await i({mode:"readwrite"})}catch{return}N=[];for await(const[t,n]of V.entries()){if(n.kind!=="file")continue;const r=((e=t.split(".").pop())==null?void 0:e.toLowerCase())??"";Tn.has(r)&&N.push({name:t,handle:n})}N.sort((t,n)=>t.name.localeCompare(n.name,"zh-Hant",{numeric:!0})),await jn(),ze=Date.now(),li(),un(),Ue(),N.length>0?(U=0,ne(0)):Gt.textContent="此資料夾沒有音檔"});function Ue(){kt.innerHTML="",N.forEach((i,e)=>{const t=document.createElement("li");e===j&&(t.className="active");const n=document.createElement("span");n.textContent=i.name,n.style.overflow="hidden",n.style.textOverflow="ellipsis",t.appendChild(n);const r=en(i.name);if(r>0){const s=document.createElement("span");s.className="badge",s.textContent=String(r),t.appendChild(s)}t.addEventListener("click",()=>void ne(e)),kt.appendChild(t)})}async function ne(i){if(i<0||i>=N.length)return;j=i,U=i,Ue();const e=N[i];Gt.textContent=e.name,tt.textContent="載入中…";const t=await e.handle.getFile();ke&&URL.revokeObjectURL(ke),ke=URL.createObjectURL(t),await D.load(ke),je(),de(),D.play()}function je(){qe.clearRegions(),ot(Fe()).filter(e=>e.time!==null).forEach((e,t)=>{qe.addRegion({start:e.time,content:String(t+1),color:Fn(e.character),drag:!1,resize:!1})})}function Kn(i,e){const t=document.createElement("div");t.className="c-rating";for(let n=1;n<=5;n++){const r=document.createElement("button");r.type="button",r.className="c-star"+(n<=i.rating?" filled":""),r.textContent="★",r.title=`${n} 分${i.rating===n?"（再按清除）":""}`,r.addEventListener("click",s=>{s.stopPropagation(),i.rating=i.rating===n?0:n,Z()}),t.appendChild(r)}e.appendChild(t)}function rn(i,e={}){const t=document.createElement("div"),n=e.badge==="🏆"?" trophy":e.badge==="👎"?" bad":"";t.className="citem"+(e.role?" role":"")+(e.focused?" rolefocus":"")+n,e.role&&e.ridx!==void 0&&(t.dataset.ridx=String(e.ridx));let r=t;if(e.role){const d=document.createElement("div");d.className="card-inner";const f=document.createElement("div");f.className="card-main";const u=document.createElement("button");u.className="cplay",u.dataset.cid=i.id,u.textContent=xe===i.id&&D.isPlaying()?"⏸":"▶",u.addEventListener("click",g=>{g.stopPropagation(),an(i,e.ridx)}),d.append(f,u),t.appendChild(d),r=f}const s=document.createElement("div");s.className="crow";const o=document.createElement("span");if(i.time===null?(o.className="ctime general",o.textContent="整體"):(o.className="ctime",o.textContent=rt(i.time),o.addEventListener("click",d=>{d.stopPropagation(),Ge(i)})),s.appendChild(o),e.role){const d=document.createElement("span");if(d.className="cfile",d.textContent=i.file,e.badge){const f=document.createElement("span");f.className="cfile-badge",f.textContent=e.badge,d.appendChild(f)}d.addEventListener("click",f=>{f.stopPropagation(),Ge(i)}),s.appendChild(d)}const l=document.createElement("span");l.className="cauthor",l.textContent=i.author,s.appendChild(l);const c=document.createElement("button");c.className="cdel",c.textContent="×",c.title="刪除",c.addEventListener("click",d=>{d.stopPropagation(),P=P.filter(f=>f.id!==i.id),Z()}),s.appendChild(c),r.appendChild(s),Kn(i,r);const a=document.createElement("div");a.className="ctext",a.textContent=i.text,a.addEventListener("dblclick",d=>{d.stopPropagation(),a.contentEditable="true",a.focus()}),a.addEventListener("keydown",d=>{d.isComposing||d.key==="Enter"&&!d.shiftKey&&(d.preventDefault(),a.blur())}),a.addEventListener("blur",()=>{a.contentEditable="false";const d=(a.innerText??"").trim();d?(i.text=d,Z()):a.textContent=i.text}),r.appendChild(a);const h=document.createElement("div");if(h.className="ctag-row",qn(i,h),r.appendChild(h),i.replies.length){const d=document.createElement("div");d.className="creplies",i.replies.forEach(f=>{const u=document.createElement("div");u.className="creply";const g=document.createElement("span");g.className="crauthor",g.textContent=f.author+"：";const p=document.createElement("span");p.textContent=f.text;const v=document.createElement("button");v.className="crdel",v.textContent="×",v.addEventListener("click",E=>{E.stopPropagation(),i.replies=i.replies.filter(S=>S.id!==f.id),Z()}),u.append(g,p,v),d.appendChild(u)}),r.appendChild(d)}const m=document.createElement("div");if(m.className="creply-input",G===i.id){const d=document.createElement("input");d.type="text",d.placeholder="回覆…（Enter 送出 / Esc 取消）",d.addEventListener("keydown",u=>{u.stopPropagation(),!u.isComposing&&(u.key==="Enter"?(u.preventDefault(),Pt(i,d.value)):u.key==="Escape"&&(u.preventDefault(),G=null,Q()))});const f=document.createElement("button");f.textContent="送出",f.addEventListener("click",u=>{u.stopPropagation(),Pt(i,d.value)}),m.append(d,f)}else{const d=document.createElement("button");d.className="creply-toggle",d.textContent="＋ 回覆",d.addEventListener("click",f=>{if(f.stopPropagation(),!te){Ve(()=>{G=i.id,Q()});return}G=i.id,Q()}),m.appendChild(d)}return r.appendChild(m),t}function qn(i,e){if(fe===i.id){const t=document.createElement("div");t.className="char-picker",ln(t,i.character,r=>{st(i.character,r),Z()});const n=document.createElement("button");n.type="button",n.className="charchip done",n.textContent="完成",n.addEventListener("click",r=>{r.stopPropagation(),fe=null,Q()}),t.appendChild(n),e.appendChild(t)}else if(i.character.length>0)i.character.forEach(t=>{const n=document.createElement("span");n.className="ctag",n.style.background=Xt(t,O)??"#3b82f6",n.textContent=t,n.addEventListener("click",r=>{r.stopPropagation(),fe=i.id,Q()}),e.appendChild(n)});else{const t=document.createElement("span");t.className="ctag empty",t.textContent="＋ 角色",t.addEventListener("click",n=>{n.stopPropagation(),fe=i.id,Q()}),e.appendChild(t)}}function de(){const i=$n();Bn.textContent=i?`留言 · ${i}`:"留言",Oe.innerHTML="";const e=ot(i);if(e.length===0){const t=document.createElement("li");t.style.color="var(--muted)",t.style.fontSize="13px",t.textContent=i?"尚無留言。播放時按 C 新增。":"尚未選擇檔案。",Oe.appendChild(t);return}e.forEach(t=>Oe.appendChild(rn(t)))}function sn(i,e){const t=[...i];return e==="rating"?t.sort((n,r)=>r.rating-n.rating||n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)):e==="file"?t.sort((n,r)=>n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)):t.sort((n,r)=>n.author.localeCompare(r.author,"zh-Hant")||n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)),t}function Jn(i){let e=P.filter(t=>t.character.includes(i));return q==="hide"?e=e.filter(t=>t.rating>0):q==="solo"&&(e=e.filter(t=>t.rating===0)),sn(e,Me)}function ce(){$.innerHTML="",B=[],$.classList.toggle("role-compact",ue);const i=document.createElement("div");i.className="role-head";const e=document.createElement("h2");e.textContent="角色 Dashboard";const t=document.createElement("button");t.className="ghost",t.textContent="返回單檔",t.addEventListener("click",()=>J("single"));const n=document.createElement("span");n.className="role-hint",n.textContent="↑↓ 移動 · 空白鍵 就地播放 · Enter 進單檔 · Tab/Esc 返回";const r=document.createElement("div");r.className="role-sort-toggle",["rating","file","user"].forEach(p=>{const v=p==="rating"?"評分":p==="file"?"檔案":"使用者",E=document.createElement("button");E.className="role-sort-btn"+(Me===p?" active":""),E.textContent=v,E.addEventListener("click",()=>{Me=p,ce()}),r.appendChild(E)});const s=document.createElement("button");s.className="role-compact-btn"+(ue?" active":""),s.textContent="簡潔",s.title="隱藏回覆與角色標籤，雙欄顯示",s.addEventListener("click",()=>{ue=!ue,$.classList.toggle("role-compact",ue),s.classList.toggle("active",ue)});const o={all:"hide",hide:"solo",solo:"all"},l={all:"隱藏 👎",hide:"solo 👎",solo:"全部留言"},c=document.createElement("button");c.className="role-zero-btn"+(q!=="all"?" "+q:""),c.textContent=l[q],c.addEventListener("click",()=>{q=o[q],ce()}),i.append(e,t,n,r,s,c);const a=document.createElement("div");a.className="role-sticky",a.appendChild(i);const h=["lead","mascot","supporting"],m=P.filter(p=>p.character.length===0).length,d=document.createElement("div");d.className="role-stats-wrap";let f=null;if(h.forEach(p=>{const v=O.filter(x=>x.role===p);if(v.length===0)return;const E=document.createElement("div");E.className="role-stats-row",p==="supporting"&&(f=E);const S=document.createElement("span");S.className="role-stats-type",S.textContent=Vt[p],S.style.color=Y[p],E.appendChild(S),v.forEach(x=>{const R=P.filter(C=>C.character.includes(x.name)).length,k=document.createElement("button");k.className="role-stat"+(R===0?" zero":""),k.style.background=Y[x.role],k.textContent=`${x.name} ${R}`,k.addEventListener("click",()=>{var C;(C=$.querySelector(`[data-char="${x.name}"]`))==null||C.scrollIntoView({behavior:"smooth",block:"start"})}),E.appendChild(k)}),d.appendChild(E)}),m>0){const p=document.createElement("button");if(p.className="role-stat",p.style.background="#64748b",p.textContent=`未指定 ${m}`,p.addEventListener("click",()=>{var v;(v=$.querySelector('[data-char="__none__"]'))==null||v.scrollIntoView({behavior:"smooth",block:"start"})}),f)f.appendChild(p);else{const v=document.createElement("div");v.className="role-stats-row",v.appendChild(p),d.appendChild(v)}}a.appendChild(d),$.appendChild(a);const u=(p,v,E)=>{if(E.length===0)return;const S=document.createElement("div");S.className="role-group",S.dataset.char=p;const x=document.createElement("div");x.className="role-group-label",x.style.background=v,x.textContent=p==="__none__"?"未指定":p,S.appendChild(x);const R=document.createElement("div");R.className="role-group-cards-wrap";const k=E.reduce((C,y)=>Math.max(C,y.rating),0);E.forEach(C=>{const y=B.length,T=C.rating===0?"👎":k>0&&C.rating===k?"🏆":void 0,b=rn(C,{role:!0,ridx:y,focused:y===W,badge:T});T==="🏆"?b.classList.add("trophy"):T==="👎"&&b.classList.add("bad"),R.appendChild(b),B.push(C)}),S.appendChild(R),$.appendChild(S)};O.forEach(p=>u(p.name,Y[p.role],Jn(p.name)));let g=P.filter(p=>p.character.length===0);if(q==="hide"?g=g.filter(p=>p.rating>0):q==="solo"&&(g=g.filter(p=>p.rating===0)),u("__none__","#64748b",sn(g,Me)),B.length===0){const p=document.createElement("div");p.style.color="var(--muted)",p.textContent="尚無留言。",$.appendChild(p)}W>=B.length&&(W=Math.max(0,B.length-1))}function Ce(){$.querySelectorAll(".cplay").forEach(i=>{i.textContent=xe===i.dataset.cid&&D.isPlaying()?"⏸":"▶"})}function on(){$.querySelectorAll(".citem").forEach(i=>{const e=i.dataset.ridx===String(W);i.classList.toggle("rolefocus",e),e&&i.scrollIntoView({block:"nearest"})})}function Dt(i){B.length!==0&&(W=Math.min(Math.max(0,W+i),B.length-1),on())}async function Gn(i,e){const t=N.findIndex(r=>r.name===i.file);if(t<0)return;t!==j&&await ne(t),D.setTime(i.time??0),D.play(),xe=i.id;const n=e!==void 0?e:B.indexOf(i);n>=0&&(W=n,on()),Ce()}function an(i,e){if(xe===i.id&&D.isPlaying()){D.pause(),Ce();return}Gn(i,e)}async function Ge(i){const e=N.findIndex(t=>t.name===i.file);e<0||(H!=="single"&&J("single"),e!==j&&await ne(e),i.time!==null&&D.setTime(i.time),D.play())}function ln(i,e,t,n){i.innerHTML="",O.forEach((r,s)=>{const o=document.createElement("button");o.type="button",o.className="charchip"+(s===n?" focused":""),o.textContent=r.name,o.style.borderColor=Y[r.role],e.includes(r.name)&&(o.style.background=Y[r.role],o.style.color="#fff"),o.addEventListener("click",l=>{l.stopPropagation(),t(r.name)}),i.appendChild(o)})}function Pt(i,e){const t=e.trim();if(!t){G=null,Q();return}const n=()=>{i.replies.push({id:Qt(),author:te,text:t,created:Date.now()}),G=null,Z()};te?n():Ve(n)}function Yn(){if(!(j<0)){if(!te){Ve(()=>Mt());return}Mt()}}function Mt(){D.pause(),et=D.getCurrentTime(),ae=!0,ge.value="",le=[],oe=0,z="text",F(),nt.classList.remove("hidden"),ge.focus()}function cn(){nt.classList.add("hidden"),z="text"}function at(){Lt.innerHTML="";for(let i=1;i<=5;i++){const e=document.createElement("button");e.type="button",e.className="c-star"+(i<=oe?" filled":""),e.textContent="★",e.title=`${i} 分`,e.addEventListener("click",t=>{t.stopPropagation(),oe=oe===i?0:i,at()}),Lt.appendChild(e)}}function F(){Zn(),dn(),at()}function Zn(){ae?(pe.textContent=`對應秒數 ${rt(et)}`,pe.classList.remove("off")):(pe.textContent="整體留言（不對應秒數）",pe.classList.add("off")),pe.classList.toggle("focused",z==="toggle")}function dn(){ln(Yt,le,e=>{st(le,e),dn()},typeof z=="number"?z:void 0)}function Tt(){const i=ge.value.trim();if(cn(),!i)return;const e=Fe();e&&(P.push({id:Qt(),file:e,time:ae?et:null,text:i,author:te,character:[...le],replies:[],rating:oe}),Z())}function Nt(){z="toggle",ge.blur(),F()}function Ot(){z="text",F(),ge.focus()}function Qn(){if(O.length===0)return;const i=le.length?O.findIndex(e=>e.name===le[0]):-1;z=i>=0?i:0,ge.blur(),F()}function ei(i){st(le,O[i].name),F()}function Le(i,e){const t=Array.from(Yt.children);if(t.length===0)return i;const n=t.map(h=>h.getBoundingClientRect()),r=n[i],s=r.left+r.width/2,o=r.top+r.height/2,l=r.height/2;if(e==="left"||e==="right"){let h=-1,m=1/0;return n.forEach((d,f)=>{if(f===i||Math.abs(d.top+d.height/2-o)>l)return;const u=d.left+d.width/2;e==="left"&&u<s&&s-u<m&&(m=s-u,h=f),e==="right"&&u>s&&u-s<m&&(m=u-s,h=f)}),h>=0?h:i}let c=-1,a=1/0;return n.forEach((h,m)=>{if(m===i)return;const d=h.top+h.height/2;if(!(e==="up"?d<o-l:d>o+l))return;const u=Math.abs(d-o)*1e3+Math.abs(h.left+h.width/2-s);u<a&&(a=u,c=m)}),e==="up"&&c<0?"toggle":c>=0?c:i}function ti(i){if(i.isComposing)return;if(i.shiftKey&&/^Digit[0-5]$/.test(i.code)){i.preventDefault();const t=parseInt(i.code[5]);oe=oe===t?0:t,at();return}if(i.code==="Escape"){i.preventDefault(),cn();return}if(z==="text"){if(Je(i)&&!i.shiftKey){i.preventDefault(),Tt();return}if(i.code==="ArrowDown"||i.code==="Tab"){i.preventDefault(),Nt();return}return}if(Je(i)){i.preventDefault(),Tt();return}if(z==="toggle"){switch(i.code){case"Space":i.preventDefault(),ae=!ae,F();break;case"ArrowUp":i.preventDefault(),Ot();break;case"ArrowDown":case"Tab":i.preventDefault(),Qn();break}return}const e=z;switch(i.code){case"Space":i.preventDefault(),ei(e);break;case"ArrowRight":{i.preventDefault();const t=Le(e,"right");typeof t=="number"&&(z=t,F());break}case"ArrowLeft":{i.preventDefault();const t=Le(e,"left");typeof t=="number"&&(z=t,F());break}case"ArrowDown":{i.preventDefault();const t=Le(e,"down");typeof t=="number"&&(z=t,F());break}case"ArrowUp":{i.preventDefault();const t=Le(e,"up");t==="toggle"?Nt():(z=t,F());break}case"Tab":i.preventDefault(),Ot();break}}pe.addEventListener("click",()=>{ae=!ae,F()});function Ve(i){Ae=i,Be.value=te,$e.classList.remove("hidden"),Be.focus()}function ni(){const i=Be.value.trim();if(!i)return;te=i,Sn(i),$e.classList.add("hidden");const e=Ae;Ae=null,e&&e()}Be.addEventListener("keydown",i=>{i.stopPropagation(),!i.isComposing&&(i.key==="Enter"?(i.preventDefault(),ni()):i.key==="Escape"&&(i.preventDefault(),$e.classList.add("hidden"),Ae=null))});Ct.addEventListener("click",()=>{Ct.blur(),be(),it.classList.remove("hidden")});function be(){St.innerHTML="",O.forEach((i,e)=>{const t=document.createElement("li"),n=document.createElement("span");n.className="dot",n.style.background=Y[i.role];const r=document.createElement("span");r.textContent=i.name,r.title="雙擊重命名",r.style.cursor="pointer",r.addEventListener("dblclick",()=>{const l=document.createElement("input");l.type="text",l.value=i.name,l.style.cssText="font-size:13px;width:120px;padding:2px 6px",t.replaceChild(l,r),l.focus(),l.select();const c=()=>{const a=l.value.trim();if(a&&a!==i.name){const h=i.name;i.name=a,P.forEach(m=>{const d=m.character.indexOf(h);d>=0&&(m.character[d]=a)}),_e(O),tn()}be()};l.addEventListener("keydown",a=>{a.stopPropagation(),a.key==="Enter"?(a.preventDefault(),c()):a.key==="Escape"&&(a.preventDefault(),be())}),l.addEventListener("blur",c)});const s=document.createElement("span");s.className="role",s.textContent=Vt[i.role];const o=document.createElement("button");o.className="rm",o.textContent="刪除",o.addEventListener("click",()=>{O.splice(e,1),_e(O),be()}),t.append(n,r,s,o),St.appendChild(t)})}Zt.addEventListener("click",()=>{const i=Ke.value.trim();i&&(O.push({name:i,role:Wn.value}),_e(O),Ke.value="",be())});Ke.addEventListener("keydown",i=>{i.stopPropagation(),!i.isComposing&&i.key==="Enter"&&(i.preventDefault(),Zt.click())});Hn.addEventListener("click",()=>{it.classList.add("hidden"),je(),de(),H==="role"&&ce()});function _t(i){const e=D.getDuration();!isFinite(e)||e===0||D.setTime(Math.min(Math.max(0,D.getCurrentTime()+i),e))}function zt(i){const e=ot(Fe()).filter(r=>r.time!==null).map(r=>r.time);if(e.length===0)return;const t=D.getCurrentTime();let n;if(i<0){for(let r=e.length-1;r>=0;r--)if(e[r]<t-Nn){n=e[r];break}n??(n=e[0])}else{for(let r=0;r<e.length;r++)if(e[r]>t+On){n=e[r];break}n??(n=e[e.length-1])}D.setTime(n)}function J(i){H=i;const e=i==="single",t=i==="grid",n=i==="role";(t||n)&&D.pause(),zn.classList.toggle("hidden",!e),An.classList.toggle("hidden",!e),Ne.classList.toggle("hidden",!t),$.classList.toggle("hidden",!n),In.classList.toggle("hidden",n),t&&hn(),n&&(W=0,ce()),de()}function hn(){Ne.innerHTML="";const i=document.createElement("div");i.className="gridhint",i.textContent="WASD／方向鍵 移動 · 空白鍵或 Enter 開啟 · Tab 返回單檔",Ne.appendChild(i),N.forEach((e,t)=>{const n=document.createElement("div");n.className="gridcard"+(t===U?" selected":""),n.innerHTML=`<div class="gc-name">${me(e.name)}</div><div class="gc-meta">${en(e.name)} 則留言</div>`,n.addEventListener("click",()=>{U=t,J("single"),ne(t)}),Ne.appendChild(n)})}function Se(i){const e=N.length;e!==0&&(U=Math.min(Math.max(0,U+i),e-1),hn(),de())}function ii(){const i=document.activeElement;return i?i.tagName==="INPUT"||i.tagName==="TEXTAREA"||i.isContentEditable:!1}function ri(){return!$e.classList.contains("hidden")||!it.classList.contains("hidden")}document.addEventListener("keydown",i=>{if(!nt.classList.contains("hidden")){ti(i);return}if(!i.isComposing){if(fe!==null&&i.code==="Escape"){i.preventDefault(),fe=null,Q();return}if(G!==null&&i.code==="Escape"){i.preventDefault(),G=null,Q();return}if(!(ri()||ii())){if(H==="role"){if(i.shiftKey&&/^Digit[0-5]$/.test(i.code)){i.preventDefault();const e=parseInt(i.code[5]),t=B[W];t&&(t.rating=t.rating===e?0:e,Z());return}if(i.code==="Tab"||i.code==="Escape"){i.preventDefault(),J("single");return}switch(i.code){case"ArrowUp":case"KeyW":i.preventDefault(),Dt(-1);break;case"ArrowDown":case"KeyS":i.preventDefault(),Dt(1);break;case"Space":i.preventDefault(),B[W]&&an(B[W],W);break;case"Enter":case"NumpadEnter":i.preventDefault(),B[W]&&Ge(B[W]);break}return}if(H==="grid"){if(Je(i)){i.preventDefault(),J("single"),ne(U);return}switch(i.code){case"KeyA":case"ArrowLeft":i.preventDefault(),Se(-1);break;case"KeyD":case"ArrowRight":i.preventDefault(),Se(1);break;case"KeyW":case"ArrowUp":i.preventDefault(),Se(-vt);break;case"KeyS":case"ArrowDown":i.preventDefault(),Se(vt);break;case"Space":i.preventDefault(),J("single"),ne(U);break;case"Tab":i.preventDefault(),J("single");break}return}switch(i.code){case"Space":i.preventDefault(),D.playPause();break;case"KeyA":case"ArrowLeft":i.preventDefault(),_t(-5);break;case"KeyD":case"ArrowRight":i.preventDefault(),_t(5);break;case"KeyW":case"ArrowUp":i.preventDefault(),zt(-1);break;case"KeyS":case"ArrowDown":i.preventDefault(),zt(1);break;case"KeyC":case"KeyX":!i.ctrlKey&&!i.metaKey&&(i.preventDefault(),Yn());break;case"Tab":i.preventDefault(),N.length&&J("grid");break}}}});function si(){let i="";const e=(s,o)=>o.rating-s.rating||s.file.localeCompare(o.file,"zh-Hant",{numeric:!0})||(s.time??-1/0)-(o.time??-1/0),t=(s,o,l)=>{if(l.length===0)return;const c=l.map(a=>{const h=a.rating>0?"★".repeat(a.rating)+"☆".repeat(5-a.rating):"",m=a.time!==null?rt(a.time):"整體";return`<tr>
        <td class="tc">${me(m)}</td>
...
</div>`};O.forEach(s=>t(s.name,Y[s.role],P.filter(o=>o.character.includes(s.name)).sort(e))),t("__none__","#64748b",P.filter(s=>s.character.length===0).sort(e));const n=`<!DOCTYPE html>
<html lang="zh-Hant">
<head>
<meta charset="UTF-8">
...
</body></html>`,r=window.open("","_blank","width=900,height=800");if(!r){alert("請允許開啟新分頁（檢查彈出視窗封鎖設定）");return}r.document.write(n),r.document.close(),setTimeout(()=>r.print(),600)}Rt.addEventListener("click",()=>{Rt.blur(),si()});const oi=1e4;function ai(i){const e=new Set(P.map(n=>n.id)),t=i.map(He).filter(n=>!e.has(n.id));return t.length===0?0:(P=[...P,...t],t.length)}function li(){At(),Pe=window.setInterval(async()=>{if(!V){At();return}try{const e=await(await V.getFileHandle(Qe)).getFile(),t=JSON.parse(await e.text());if(!Array.isArray(t==null?void 0:t.comments))return;const n=ai(t.comments);ze=Date.now(),un(),n>0&&(Ze(P),je(),Ue(),de(),H==="role"&&ce(),ci(`已同步 ${n} 則新留言`))}catch{}},oi)}function At(){Pe!==void 0&&(window.clearInterval(Pe),Pe=void 0)}function un(){const i=document.getElementById("syncStatus");if(!i)return;if(!V){i.textContent="";return}const e=ze?new Date(ze).toLocaleTimeString("zh-Hant",{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"—";i.textContent=`↺ 協作同步中 · ${e}`}let It;function ci(i){const e=document.getElementById("syncToast");e&&(e.textContent=i,e.classList.remove("hidden"),window.clearTimeout(It),It=window.setTimeout(()=>e.classList.add("hidden"),3e3))}async function di(){if(N.length===0){alert("請先選擇音檔資料夾後再匯出外部分享版。");return}const i=new Set(P.map(m=>m.file)),e=N.filter(m=>i.has(m.name));if(e.length===0){alert("目前沒有任何留言，無法產生分享版。");return}se.disabled=!0,se.textContent=`處理中 0/${e.length}…`;const t={};for(let m=0;m<e.length;m++){const d=e[m];se.textContent=`處理中 ${m+1}/${e.length}…`;try{const f=await d.handle.getFile(),u=await new Promise((g,p)=>{const v=new FileReader;v.onload=()=>g(v.result),v.onerror=p,v.readAsDataURL(f)});t[d.name]=u}catch{}}se.disabled=!1,se.textContent="匯出分享版";const n=m=>JSON.stringify(m).replace(/</g,"\\u003c").replace(/>/g,"\\u003e").replace(/&/g,"\\u0026"),r=Mn.replace("{{DATA_JSON}}",()=>n({comments:P,characters:O,roleColors:Y})).replace("{{AUDIO_JSON}}",()=>n(t)),s=new Blob([r],{type:"text/html;charset=utf-8"}),o=URL.createObjectURL(s),l=new Date,c=m=>String(m).padStart(2,"0"),a=`${l.getFullYear()}${c(l.getMonth()+1)}${c(l.getDate())}-${c(l.getHours())}${c(l.getMinutes())}`,h=document.createElement("a");h.href=o,h.download=`voicepicker-external-${a}.html`,h.click(),URL.revokeObjectURL(o)}se.addEventListener("click",()=>{se.blur(),di()});wt.addEventListener("click",()=>{wt.blur(),J("role")});_n.textContent="v0.9.6";te||Ve(null);
