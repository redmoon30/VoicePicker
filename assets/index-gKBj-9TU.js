...
/* 角色快捷列 + 圖例橫排 */
.stats-legend-row { display: flex; align-items: flex-start; gap: 20px; margin-bottom: 16px; }
.role-stats-wrap { display: flex; flex-direction: column; gap: 7px; flex: 1; min-width: 0; }
...
.role-group-cards-wrap { flex: 1; min-width: 0; padding: 8px; display: grid; grid-template-columns: repeat(2, 1fr); gap: 6px; }
...
/* 簡潔模式（永久）：隱藏回覆與角色標籤 */
.creplies, .ctag-row { display: none !important; }
...
function renderRoleView() {
  ...
  const statsLegendRow = document.createElement('div');
  statsLegendRow.className = 'stats-legend-row';
  statsLegendRow.appendChild(statsWrap);
  const legendEl = document.createElement('div');
  legendEl.className = 'legend';
  legendEl.innerHTML =
    '<div class="legend-item"><span class="legend-badge">🏆</span><span>相對最適合的方案，分數代表契合度。可參考我們的挑選原因</span></div>' +
    '<div class="legend-item"><span class="legend-badge">👎</span><span>我們認為不適合的方向，或是希望避開的聲音表演元素（可按右上角按鈕顯示或隱藏）</span></div>';
  statsLegendRow.appendChild(legendEl);
  roleviewEl.appendChild(statsLegendRow);
