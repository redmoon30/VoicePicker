...
`,An=new Set(["wav","mp3","m4a","aac","ogg","flac","wma","opus"]),Et=4,In=1.5,Bn=.25,tt="voicepicker.json";let N=[],j=-1,ke=null,H="single",U=0,X=null,xt,Pe,Ae=null,te=Mn(),P=On(),O=_n(),nt=0,ae=!0,le=[],z="text",fe=null,Y=null,Ie=null,B=[],W=0,xe=null,Me="rating",ue=!1,J="all",oe=0;const Wn=50,Te=[];function it(){Te.push(JSON.parse(JSON.stringify(P))),Te.length>Wn&&Te.shift()}const w=i=>document.getElementById(i),Ct=w("pick"),wt=w("exportBtn"),kt=w("importBtn"),Ne=w("importFile"),Lt=w("editChars"),St=w("roleViewBtn"),Rt=w("filelist"),tn=w("nowplaying"),rt=w("status"),Be=w("playBtn"),Hn=w("appVer"),$n=w("sidebar"),Fn=w("player"),Oe=w("grid"),$=w("roleview"),Un=w("comments"),jn=w("commentsHead"),_e=w("commentlist"),st=w("composer"),pe=w("composerToggle"),ge=w("composerText"),nn=w("composerChars"),Dt=w("composerRatingEl"),Fe=w("nameModal"),We=w("nameInput"),ot=w("charModal"),Pt=w("charEditList"),qe=w("newCharName"),Vn=w("newCharRole"),rn=w("addCharBtn"),Xn=w("closeCharBtn"),Mt=w("ratingsBtn"),se=w("shareBtn"),D=ye.create({container:"#waveform",waveColor:"#7a8ba6",progressColor:"#3b82f6",cursorColor:"#ef4444",height:120}),Je=D.registerPlugin(Qe.create());D.on("ready",()=>{rt.textContent=`${He(0)} / ${He(D.getDuration())}`});D.on("timeupdate",i=>{rt.textContent=`${He(i)} / ${He(D.getDuration())}`});D.on("play",()=>{Be.textContent="⏸ 暫停",H==="role"&&Ce()});D.on("pause",()=>{Be.textContent="▶ 播放",H==="role"&&Ce()});D.on("finish",()=>{H==="single"&&j<N.length-1?ne(j+1):H==="role"&&(xe=null,Ce())});Je.on("region-clicked",(i,e)=>{e.stopPropagation(),D.setTime(i.start),D.play()});Be.addEventListener("click",()=>{Be.blur(),D.playPause()});function at(i){if(!isFinite(i))return"0:00";const e=Math.floor(i/60),t=Math.floor(i%60).toString().padStart(2,"0");return`${e}:${t}`}function He(i){if(!isFinite(i))return"0:00.00";const e=Math.floor(i/60),t=Math.floor(i%60),n=Math.floor((i-Math.floor(i))*100);return`${e}:${t.toString().padStart(2,"0")}.${n.toString().padStart(2,"0")}`}function sn(){return Math.random().toString(36).slice(2)+Date.now().toString(36)}function Ge(i){return i.code==="Enter"||i.code==="NumpadEnter"}function lt(i,e){const t=i.indexOf(e);t>=0?i.splice(t,1):i.push(e)}function me(i){return i.replace(/[&<>"']/g,e=>({"&":"&amp;","<":"&lt;",">":"&gt;",'"':"&quot;","'":"&#39;"})[e])}function Ue(){return j>=0&&j<N.length?N[j].name:null}function Kn(){return H==="grid"?U>=0&&U<N.length?N[U].name:null:Ue()}function ct(i){return i?P.filter(e=>e.file===i).sort((e,t)=>(e.time??-1/0)-(t.time??-1/0)):[]}function on(i){return P.filter(e=>e.file===i).length}function qn(i){return(i.length?Yt(i[0],O):null)??"#3b82f6"}function Jn(){const i=Te.pop();if(!i){Ze("沒有可還原的操作");return}P=i,V(),Ze("已還原")}function V(){an(),Ve(),je(),de(),H==="role"&&ce()}function Q(){var i;de(),H==="role"&&ce(),Y&&((i=(H==="role"?$:_e).querySelector(".creply-input input"))==null||i.focus())}function an(){et(P),X&&(window.clearTimeout(xt),xt=window.setTimeout(()=>void Gn(),300))}async function Gn(){if(X)try{const e=await(await X.getFileHandle(tt,{create:!0})).createWritable();await e.write(JSON.stringify({version:1,updated:new Date().toISOString(),comments:P},null,2)),await e.close()}catch{}}function ln(i){const e=new Map(P.map(t=>[t.id,t]));i.forEach(t=>e.set(t.id,t)),P=[...e.values()]}async function Yn(){if(X)try{const e=await(await X.getFileHandle(tt)).getFile(),t=JSON.parse(await e.text());Array.isArray(t==null?void 0:t.comments)&&(ln(t.comments.map($e)),et(P))}catch{}}function Zn(){const i=JSON.stringify({version:1,exported:new Date().toISOString(),comments:P,characters:O},null,2),e=new Blob([i],{type:"application/json"}),t=URL.createObjectURL(e),n=new Date,r=l=>String(l).padStart(2,"0"),s=`${n.getFullYear()}${r(n.getMonth()+1)}${r(n.getDate())}-${r(n.getHours())}${r(n.getMinutes())}`,o=document.createElement("a");o.href=t,o.download=`voicepicker-backup-${s}.json`,o.click(),URL.revokeObjectURL(t)}async function Qn(i){try{const e=JSON.parse(await i.text());Array.isArray(e==null?void 0:e.comments)&&ln(e.comments.map($e)),Array.isArray(e==null?void 0:e.characters)&&e.characters.length>0&&(O=e.characters,ze(O)),V(),alert(`匯入完成：目前共 ${P.length} 則留言。`)}catch{alert("匯入失敗：檔案不是有效的 VoicePicker 備份。")}}wt.addEventListener("click",()=>{wt.blur(),Zn()});kt.addEventListener("click",()=>{kt.blur(),Ne.click()});Ne.addEventListener("change",()=>{var e;const i=(e=Ne.files)==null?void 0:e[0];i&&Qn(i),Ne.value=""});Ct.addEventListener("click",async()=>{var e;Ct.blur();const i=window.showDirectoryPicker;if(!i){alert("此瀏覽器不支援資料夾選取，請改用 Chrome / Edge。");return}try{X=await i({mode:"readwrite"})}catch{return}N=[];for await(const[t,n]of X.entries()){if(n.kind!=="file")continue;const r=((e=t.split(".").pop())==null?void 0:e.toLowerCase())??"";An.has(r)&&N.push({name:t,handle:n})}N.sort((t,n)=>t.name.localeCompare(n.name,"zh-Hant",{numeric:!0})),await Yn(),Ae=Date.now(),pi(),vn(),je(),N.length>0?(U=0,ne(0)):tn.textContent="此資料夾沒有音檔"});function je(){Rt.innerHTML="",N.forEach((i,e)=>{const t=document.createElement("li");e===j&&(t.className="active");const n=document.createElement("span");n.textContent=i.name,n.style.overflow="hidden",n.style.textOverflow="ellipsis",t.appendChild(n);const r=on(i.name);if(r>0){const s=document.createElement("span");s.className="badge",s.textContent=String(r),t.appendChild(s)}t.addEventListener("click",()=>void ne(e)),Rt.appendChild(t)})}async function ne(i){if(i<0||i>=N.length)return;j=i,U=i,je();const e=N[i];tn.textContent=e.name,rt.textContent="載入中…";const t=await e.handle.getFile();ke&&URL.revokeObjectURL(ke),ke=URL.createObjectURL(t),await D.load(ke),Ve(),de(),D.play()}function Ve(){Je.clearRegions(),ct(Ue()).filter(e=>e.time!==null).forEach((e,t)=>{Je.addRegion({start:e.time,content:String(t+1),color:qn(e.character),drag:!1,resize:!1})})}function ei(i,e){const t=document.createElement("div");t.className="c-rating";for(let n=1;n<=5;n++){const r=document.createElement("button");r.type="button",r.className="c-star"+(n<=i.rating?" filled":""),r.textContent="★",r.title=`${n} 分${n===1?"（1星再按清除）":""}`,r.addEventListener("click",s=>{s.stopPropagation(),it(),i.rating=n===1&&i.rating===1?0:n,V()}),t.appendChild(r)}e.appendChild(t)}function cn(i,e={}){const t=document.createElement("div"),n=e.badge==="🏆"?" trophy":e.badge==="👎"?" bad":"";t.className="citem"+(e.role?" role":"")+(e.focused?" rolefocus":"")+n,e.role&&e.ridx!==void 0&&(t.dataset.ridx=String(e.ridx));let r=t;if(e.role){const d=document.createElement("div");d.className="card-inner";const f=document.createElement("div");f.className="card-main";const u=document.createElement("button");u.className="cplay",u.dataset.cid=i.id,u.textContent=xe===i.id&&D.isPlaying()?"⏸":"▶",u.addEventListener("click",g=>{g.stopPropagation(),un(i,e.ridx)}),d.append(f,u),t.appendChild(d),r=f}const s=document.createElement("div");s.className="crow";const o=document.createElement("span");if(i.time===null?(o.className="ctime general",o.textContent="整體"):(o.className="ctime",o.textContent=at(i.time),o.addEventListener("click",d=>{d.stopPropagation(),Ye(i)})),s.appendChild(o),e.role){const d=document.createElement("span");if(d.className="cfile",d.textContent=i.file,e.badge){const f=document.createElement("span");f.className="cfile-badge",f.textContent=e.badge,d.appendChild(f)}d.addEventListener("click",f=>{f.stopPropagation(),Ye(i)}),s.appendChild(d)}const l=document.createElement("span");l.className="cauthor",l.textContent=i.author,s.appendChild(l);const c=document.createElement("button");c.className="cdel",c.textContent="×",c.title="刪除",c.addEventListener("click",d=>{d.stopPropagation(),it(),P=P.filter(f=>f.id!==i.id),V()}),s.appendChild(c),r.appendChild(s),ei(i,r);const a=document.createElement("div");a.className="ctext",a.textContent=i.text,a.addEventListener("dblclick",d=>{d.stopPropagation(),a.contentEditable="true",a.focus()}),a.addEventListener("keydown",d=>{d.isComposing||d.key==="Enter"&&!d.shiftKey&&(d.preventDefault(),a.blur())}),a.addEventListener("blur",()=>{a.contentEditable="false";const d=(a.innerText??"").trim();d?(i.text=d,V()):a.textContent=i.text}),r.appendChild(a);const h=document.createElement("div");if(h.className="ctag-row",ti(i,h),r.appendChild(h),i.replies.length){const d=document.createElement("div");d.className="creplies",i.replies.forEach(f=>{const u=document.createElement("div");u.className="creply";const g=document.createElement("span");g.className="crauthor",g.textContent=f.author+"：";const p=document.createElement("span");p.textContent=f.text;const v=document.createElement("button");v.className="crdel",v.textContent="×",v.addEventListener("click",E=>{E.stopPropagation(),i.replies=i.replies.filter(S=>S.id!==f.id),V()}),u.append(g,p,v),d.appendChild(u)}),r.appendChild(d)}const m=document.createElement("div");if(m.className="creply-input",Y===i.id){const d=document.createElement("input");d.type="text",d.placeholder="回覆…（Enter 送出 / Esc 取消）",d.addEventListener("keydown",u=>{u.stopPropagation(),!u.isComposing&&(u.key==="Enter"?(u.preventDefault(),Nt(i,d.value)):u.key==="Escape"&&(u.preventDefault(),Y=null,Q()))});const f=document.createElement("button");f.textContent="送出",f.addEventListener("click",u=>{u.stopPropagation(),Nt(i,d.value)}),m.append(d,f)}else{const d=document.createElement("button");d.className="creply-toggle",d.textContent="＋ 回覆",d.addEventListener("click",f=>{if(f.stopPropagation(),!te){Xe(()=>{Y=i.id,Q()});return}Y=i.id,Q()}),m.appendChild(d)}return r.appendChild(m),t}function ti(i,e){if(fe===i.id){const t=document.createElement("div");t.className="char-picker",pn(t,i.character,r=>{lt(i.character,r),V()});const n=document.createElement("button");n.type="button",n.className="charchip done",n.textContent="完成",n.addEventListener("click",r=>{r.stopPropagation(),fe=null,Q()}),t.appendChild(n),e.appendChild(t)}else if(i.character.length>0)i.character.forEach(t=>{const n=document.createElement("span");n.className="ctag",n.style.background=Yt(t,O)??"#3b82f6",n.textContent=t,n.addEventListener("click",r=>{r.stopPropagation(),fe=i.id,Q()}),e.appendChild(n)});else{const t=document.createElement("span");t.className="ctag empty",t.textContent="＋ 角色",t.addEventListener("click",n=>{n.stopPropagation(),fe=i.id,Q()}),e.appendChild(t)}}function de(){const i=Kn();jn.textContent=i?`留言 · ${i}`:"留言",_e.innerHTML="";const e=ct(i);if(e.length===0){const t=document.createElement("li");t.style.color="var(--muted)",t.style.fontSize="13px",t.textContent=i?"尚無留言。播放時按 C 新增。":"尚未選擇檔案。",_e.appendChild(t);return}e.forEach(t=>_e.appendChild(cn(t)))}function dn(i,e){const t=[...i];return e==="rating"?t.sort((n,r)=>r.rating-n.rating||n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)):e==="file"?t.sort((n,r)=>n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)):t.sort((n,r)=>n.author.localeCompare(r.author,"zh-Hant")||n.file.localeCompare(r.file,"zh-Hant",{numeric:!0})||(n.time??-1/0)-(r.time??-1/0)),t}function ni(i){let e=P.filter(t=>t.character.includes(i));return J==="hide"?e=e.filter(t=>t.rating>0):J==="solo"&&(e=e.filter(t=>t.rating===0)),dn(e,Me)}function ce(){$.innerHTML="",B=[],$.classList.toggle("role-compact",ue);const i=document.createElement("div");i.className="role-head";const e=document.createElement("h2");e.textContent="角色 Dashboard";const t=document.createElement("button");t.className="ghost",t.textContent="返回單檔",t.addEventListener("click",()=>G("single"));const n=document.createElement("span");n.className="role-hint",n.textContent="↑↓ 移動 · 空白鍵 就地播放 · Enter 進單檔 · Tab/Esc 返回";const r=document.createElement("div");r.className="role-sort-toggle",["rating","file","user"].forEach(p=>{const v=p==="rating"?"評分":p==="file"?"檔案":"使用者",E=document.createElement("button");E.className="role-sort-btn"+(Me===p?" active":""),E.textContent=v,E.addEventListener("click",()=>{Me=p,ce()}),r.appendChild(E)});const s=document.createElement("button");s.className="role-compact-btn"+(ue?" active":""),s.textContent="簡潔",s.title="隱藏回覆與角色標籤，雙欄顯示",s.addEventListener("click",()=>{ue=!ue,$.classList.toggle("role-compact",ue),s.classList.toggle("active",ue)});const o={all:"hide",hide:"solo",solo:"all"},l={all:"隱藏 👎",hide:"solo 👎",solo:"全部留言"},c=document.createElement("button");c.className="role-zero-btn"+(J!=="all"?" "+J:""),c.textContent=l[J],c.addEventListener("click",()=>{J=o[J],ce()}),i.append(e,t,n,r,s,c);const a=document.createElement("div");a.className="role-sticky",a.appendChild(i);const h=["lead","mascot","supporting"],m=P.filter(p=>p.character.length===0).length,d=document.createElement("div");d.className="role-stats-wrap";let f=null;if(h.forEach(p=>{const v=O.filter(x=>x.role===p);if(v.length===0)return;const E=document.createElement("div");E.className="role-stats-row",p==="supporting"&&(f=E);const S=document.createElement("span");S.className="role-stats-type",S.textContent=Gt[p],S.style.color=Z[p],E.appendChild(S),v.forEach(x=>{const R=P.filter(C=>C.character.includes(x.name)).length,k=document.createElement("button");k.className="role-stat"+(R===0?" zero":""),k.style.background=Z[x.role],k.textContent=`${x.name} ${R}`,k.addEventListener("click",()=>{var C;(C=$.querySelector(`[data-char="${x.name}"]`))==null||C.scrollIntoView({behavior:"smooth",block:"start"})}),E.appendChild(k)}),d.appendChild(E)}),m>0){const p=document.createElement("button");if(p.className="role-stat",p.style.background="#64748b",p.textContent=`未指定 ${m}`,p.addEventListener("click",()=>{var v;(v=$.querySelector('[data-char="__none__"]'))==null||v.scrollIntoView({behavior:"smooth",block:"start"})}),f)f.appendChild(p);else{const v=document.createElement("div");v.className="role-stats-row",v.appendChild(p),d.appendChild(v)}}a.appendChild(d),$.appendChild(a);const u=(p,v,E)=>{if(E.length===0)return;const S=document.createElement("div");S.className="role-group",S.dataset.char=p;const x=document.createElement("div");x.className="role-group-label",x.style.background=v,x.textContent=p==="__none__"?"未指定":p,S.appendChild(x);const R=document.createElement("div");R.className="role-group-cards-wrap";const k=E.reduce((C,y)=>Math.max(C,y.rating),0);E.forEach(C=>{const y=B.length,T=C.rating===0?"👎":k>0&&C.rating===k?"🏆":void 0,b=cn(C,{role:!0,ridx:y,focused:y===W,badge:T});T==="🏆"?b.classList.add("trophy"):T==="👎"&&b.classList.add("bad"),R.appendChild(b),B.push(C)}),S.appendChild(R),$.appendChild(S)};O.forEach(p=>u(p.name,Z[p.role],ni(p.name)));let g=P.filter(p=>p.character.length===0);if(J==="hide"?g=g.filter(p=>p.rating>0):J==="solo"&&(g=g.filter(p=>p.rating===0)),u("__none__","#64748b",dn(g,Me)),B.length===0){const p=document.createElement("div");p.style.color="var(--muted)",p.textContent="尚無留言。",$.appendChild(p)}W>=B.length&&(W=Math.max(0,B.length-1))}function Ce(){$.querySelectorAll(".cplay").forEach(i=>{i.textContent=xe===i.dataset.cid&&D.isPlaying()?"⏸":"▶"})}function hn(){$.querySelectorAll(".citem").forEach(i=>{const e=i.dataset.ridx===String(W);i.classList.toggle("rolefocus",e),e&&i.scrollIntoView({block:"nearest"})})}function Tt(i){B.length!==0&&(W=Math.min(Math.max(0,W+i),B.length-1),hn())}async function ii(i,e){const t=N.findIndex(r=>r.name===i.file);if(t<0)return;t!==j&&await ne(t),D.setTime(i.time??0),D.play(),xe=i.id;const n=e!==void 0?e:B.indexOf(i);n>=0&&(W=n,hn()),Ce()}function un(i,e){if(xe===i.id&&D.isPlaying()){D.pause(),Ce();return}ii(i,e)}async function Ye(i){const e=N.findIndex(t=>t.name===i.file);e<0||(H!=="single"&&G("single"),e!==j&&await ne(e),i.time!==null&&D.setTime(i.time),D.play())}function pn(i,e,t,n){i.innerHTML="",O.forEach((r,s)=>{const o=document.createElement("button");o.type="button",o.className="charchip"+(s===n?" focused":""),o.textContent=r.name,o.style.borderColor=Z[r.role],e.includes(r.name)&&(o.style.background=Z[r.role],o.style.color="#fff"),o.addEventListener("click",l=>{l.stopPropagation(),t(r.name)}),i.appendChild(o)})}function Nt(i,e){const t=e.trim();if(!t){Y=null,Q();return}const n=()=>{i.replies.push({id:sn(),author:te,text:t,created:Date.now()}),Y=null,V()};te?n():Xe(n)}function ri(){if(!(j<0)){if(!te){Xe(()=>Ot());return}Ot()}}function Ot(){D.pause(),nt=D.getCurrentTime(),ae=!0,ge.value="",le=[],oe=0,z="text",F(),st.classList.remove("hidden"),ge.focus()}function mn(){st.classList.add("hidden"),z="text"}function dt(){Dt.innerHTML="";for(let i=1;i<=5;i++){const e=document.createElement("button");e.type="button",e.className="c-star"+(i<=oe?" filled":""),e.textContent="★",e.title=`${i} 分`,e.addEventListener("click",t=>{t.stopPropagation(),oe=i===1&&oe===1?0:i,dt()}),Dt.appendChild(e)}}function F(){si(),fn(),dt()}function si(){ae?(pe.textContent=`對應秒數 ${at(nt)}`,pe.classList.remove("off")):(pe.textContent="整體留言（不對應秒數）",pe.classList.add("off")),pe.classList.toggle("focused",z==="toggle")}function fn(){pn(nn,le,e=>{lt(le,e),fn()},typeof z=="number"?z:void 0)}function _t(){const i=ge.value.trim();if(mn(),!i)return;const e=Ue();e&&(P.push({id:sn(),file:e,time:ae?nt:null,text:i,author:te,character:[...le],replies:[],rating:oe}),V())}function zt(){z="toggle",ge.blur(),F()}function At(){z="text",F(),ge.focus()}function oi(){if(O.length===0)return;const i=le.length?O.findIndex(e=>e.name===le[0]):-1;z=i>=0?i:0,ge.blur(),F()}function ai(i){lt(le,O[i].name),F()}function Le(i,e){const t=Array.from(nn.children);if(t.length===0)return i;const n=t.map(h=>h.getBoundingClientRect()),r=n[i],s=r.left+r.width/2,o=r.top+r.height/2,l=r.height/2;if(e==="left"||e==="right"){let h=-1,m=1/0;return n.forEach((d,f)=>{if(f===i||Math.abs(d.top+d.height/2-o)>l)return;const u=d.left+d.width/2;e==="left"&&u<s&&s-u<m&&(m=s-u,h=f),e==="right"&&u>s&&u-s<m&&(m=u-s,h=f)}),h>=0?h:i}let c=-1,a=1/0;return n.forEach((h,m)=>{if(m===i)return;const d=h.top+h.height/2;if(!(e==="up"?d<o-l:d>o+l))return;const u=Math.abs(d-o)*1e3+Math.abs(h.left+h.width/2-s);u<a&&(a=u,c=m)}),e==="up"&&c<0?"toggle":c>=0?c:i}function li(i){if(i.isComposing)return;if(i.shiftKey&&/^Digit[0-5]$/.test(i.code)){i.preventDefault();const t=parseInt(i.code[5]);oe=t===0||t===1&&oe===1?0:t,dt();return}if(i.code==="Escape"){i.preventDefault(),mn();return}if(z==="text"){if(Ge(i)&&!i.shiftKey){i.preventDefault(),_t();return}if(i.code==="ArrowDown"||i.code==="Tab"){i.preventDefault(),zt();return}return}if(Ge(i)){i.preventDefault(),_t();return}if(z==="toggle"){switch(i.code){case"Space":i.preventDefault(),ae=!ae,F();break;case"ArrowUp":i.preventDefault(),At();break;case"ArrowDown":case"Tab":i.preventDefault(),oi();break}return}const e=z;switch(i.code){case"Space":i.preventDefault(),ai(e);break;case"ArrowRight":{i.preventDefault();const t=Le(e,"right");typeof t=="number"&&(z=t,F());break}case"ArrowLeft":{i.preventDefault();const t=Le(e,"left");typeof t=="number"&&(z=t,F());break}case"ArrowDown":{i.preventDefault();const t=Le(e,"down");typeof t=="number"&&(z=t,F());break}case"ArrowUp":{i.preventDefault();const t=Le(e,"up");t==="toggle"?zt():(z=t,F());break}case"Tab":i.preventDefault(),At();break}}pe.addEventListener("click",()=>{ae=!ae,F()});function Xe(i){Ie=i,We.value=te,Fe.classList.remove("hidden"),We.focus()}function ci(){const i=We.value.trim();if(!i)return;te=i,Tn(i),Fe.classList.add("hidden");const e=Ie;Ie=null,e&&e()}We.addEventListener("keydown",i=>{i.stopPropagation(),!i.isComposing&&(i.key==="Enter"?(i.preventDefault(),ci()):i.key==="Escape"&&(i.preventDefault(),Fe.classList.add("hidden"),Ie=null))});Lt.addEventListener("click",()=>{Lt.blur(),be(),ot.classList.remove("hidden")});function be(){Pt.innerHTML="",O.forEach((i,e)=>{const t=document.createElement("li"),n=document.createElement("span");n.className="dot",n.style.background=Z[i.role];const r=document.createElement("span");r.textContent=i.name,r.title="雙擊重命名",r.style.cursor="pointer",r.addEventListener("dblclick",()=>{const l=document.createElement("input");l.type="text",l.value=i.name,l.style.cssText="font-size:13px;width:120px;padding:2px 6px",t.replaceChild(l,r),l.focus(),l.select();const c=()=>{const a=l.value.trim();if(a&&a!==i.name){const h=i.name;i.name=a,P.forEach(m=>{const d=m.character.indexOf(h);d>=0&&(m.character[d]=a)}),ze(O),an()}be()};l.addEventListener("keydown",a=>{a.stopPropagation(),a.key==="Enter"?(a.preventDefault(),c()):a.key==="Escape"&&(a.preventDefault(),be())}),l.addEventListener("blur",c)});const s=document.createElement("span");s.className="role",s.textContent=Gt[i.role];const o=document.createElement("button");o.className="rm",o.textContent="刪除",o.addEventListener("click",()=>{O.splice(e,1),ze(O),be()}),t.append(n,r,s,o),Pt.appendChild(t)})}rn.addEventListener("click",()=>{const i=qe.value.trim();i&&(O.push({name:i,role:Vn.value}),ze(O),qe.value="",be())});qe.addEventListener("keydown",i=>{i.stopPropagation(),!i.isComposing&&i.key==="Enter"&&(i.preventDefault(),rn.click())});Xn.addEventListener("click",()=>{ot.classList.add("hidden"),Ve(),de(),H==="role"&&ce()});function It(i){const e=D.getDuration();!isFinite(e)||e===0||D.setTime(Math.min(Math.max(0,D.getCurrentTime()+i),e))}function Bt(i){const e=ct(Ue()).filter(r=>r.time!==null).map(r=>r.time);if(e.length===0)return;const t=D.getCurrentTime();let n;if(i<0){for(let r=e.length-1;r>=0;r--)if(e[r]<t-In){n=e[r];break}n??(n=e[0])}else{for(let r=0;r<e.length;r++)if(e[r]>t+Bn){n=e[r];break}n??(n=e[e.length-1])}D.setTime(n)}function G(i){H=i;const e=i==="single",t=i==="grid",n=i==="role";(t||n)&&D.pause(),$n.classList.toggle("hidden",!e),Fn.classList.toggle("hidden",!e),Oe.classList.toggle("hidden",!t),$.classList.toggle("hidden",!n),Un.classList.toggle("hidden",n),t&&gn(),n&&(W=0,ce()),de()}function gn(){Oe.innerHTML="";const i=document.createElement("div");i.className="gridhint",i.textContent="WASD／方向鍵 移動 · 空白鍵或 Enter 開啟 · Tab 返回單檔",Oe.appendChild(i),N.forEach((e,t)=>{const n=document.createElement("div");n.className="gridcard"+(t===U?" selected":""),n.innerHTML=`<div class="gc-name">${me(e.name)}</div><div class="gc-meta">${on(e.name)} 則留言</div>`,n.addEventListener("click",()=>{U=t,G("single"),ne(t)}),Oe.appendChild(n)})}function Se(i){const e=N.length;e!==0&&(U=Math.min(Math.max(0,U+i),e-1),gn(),de())}function Wt(){const i=document.activeElement;return i?i.tagName==="INPUT"||i.tagName==="TEXTAREA"||i.isContentEditable:!1}function Ht(){return!Fe.classList.contains("hidden")||!ot.classList.contains("hidden")}document.addEventListener("keydown",i=>{if(!st.classList.contains("hidden")){li(i);return}if(!i.isComposing){if(fe!==null&&i.code==="Escape"){i.preventDefault(),fe=null,Q();return}if(Y!==null&&i.code==="Escape"){i.preventDefault(),Y=null,Q();return}if((i.ctrlKey||i.metaKey)&&i.code==="KeyZ"&&!i.shiftKey){!Ht()&&!Wt()&&(i.preventDefault(),Jn());return}if(!(Ht()||Wt())){if(H==="role"){if(i.shiftKey&&/^Digit[0-5]$/.test(i.code)){i.preventDefault();const e=parseInt(i.code[5]),t=B[W];t&&(it(),t.rating=e===0||e===1&&t.rating===1?0:e,V());return}if(i.code==="Tab"||i.code==="Escape"){i.preventDefault(),G("single");return}switch(i.code){case"ArrowUp":case"KeyW":i.preventDefault(),Tt(-1);break;case"ArrowDown":case"KeyS":i.preventDefault(),Tt(1);break;case"Space":i.preventDefault(),B[W]&&un(B[W],W);break;case"Enter":case"NumpadEnter":i.preventDefault(),B[W]&&Ye(B[W]);break}return}if(H==="grid"){if(Ge(i)){i.preventDefault(),G("single"),ne(U);return}switch(i.code){case"KeyA":case"ArrowLeft":i.preventDefault(),Se(-1);break;case"KeyD":case"ArrowRight":i.preventDefault(),Se(1);break;case"KeyW":case"ArrowUp":i.preventDefault(),Se(-Et);break;case"KeyS":case"ArrowDown":i.preventDefault(),Se(Et);break;case"Space":i.preventDefault(),G("single"),ne(U);break;case"Tab":i.preventDefault(),G("single");break}return}switch(i.code){case"Space":i.preventDefault(),D.playPause();break;case"KeyA":case"ArrowLeft":i.preventDefault(),It(-5);break;case"KeyD":case"ArrowRight":i.preventDefault(),It(5);break;case"KeyW":case"ArrowUp":i.preventDefault(),Bt(-1);break;case"KeyS":case"ArrowDown":i.preventDefault(),Bt(1);break;case"KeyC":case"KeyX":!i.ctrlKey&&!i.metaKey&&(i.preventDefault(),ri());break;case"Tab":i.preventDefault(),N.length&&G("grid");break}}}});function di(){let i="";const e=(s,o)=>o.rating-s.rating||s.file.localeCompare(o.file,"zh-Hant",{numeric:!0})||(s.time??-1/0)-(o.time??-1/0),t=(s,o,l)=>{if(l.length===0)return;const c=l.map(a=>{const h=a.rating>0?"★".repeat(a.rating)+"☆".repeat(5-a.rating):"",m=a.time!==null?at(a.time):"整體";return`<tr>
        <td class="tc">${me(m)}</td>
...
</body></html>`,r=window.open("","_blank","width=900,height=800");if(!r){alert("請允許開啟新分頁（檢查彈出視窗封鎖設定）");return}r.document.write(n),r.document.close(),setTimeout(()=>r.print(),600)}Mt.addEventListener("click",()=>{Mt.blur(),di()});const hi=1e4;function ui(i){const e=new Set(P.map(n=>n.id)),t=i.map($e).filter(n=>!e.has(n.id));return t.length===0?0:(P=[...P,...t],t.length)}function pi(){$t(),Pe=window.setInterval(async()=>{if(!X){$t();return}try{const e=await(await X.getFileHandle(tt)).getFile(),t=JSON.parse(await e.text());if(!Array.isArray(t==null?void 0:t.comments))return;const n=ui(t.comments);Ae=Date.now(),vn(),n>0&&(et(P),Ve(),je(),de(),H==="role"&&ce(),Ze(`已同步 ${n} 則新留言`))}catch{}},hi)}function $t(){Pe!==void 0&&(window.clearInterval(Pe),Pe=void 0)}function vn(){const i=document.getElementById("syncStatus");if(!i)return;if(!X){i.textContent="";return}const e=Ae?new Date(Ae).toLocaleTimeString("zh-Hant",{hour:"2-digit",minute:"2-digit",second:"2-digit"}):"—";i.textContent=`↺ 協作同步中 · ${e}`}let Ft;function Ze(i){const e=document.getElementById("syncToast");e&&(e.textContent=i,e.classList.remove("hidden"),window.clearTimeout(Ft),Ft=window.setTimeout(()=>e.classList.add("hidden"),3e3))}async function mi(){if(N.length===0){alert("請先選擇音檔資料夾後再匯出外部分享版。");return}const i=new Set(P.map(m=>m.file)),e=N.filter(m=>i.has(m.name));if(e.length===0){alert("目前沒有任何留言，無法產生分享版。");return}se.disabled=!0,se.textContent=`處理中 0/${e.length}…`;const t={};for(let m=0;m<e.length;m++){const d=e[m];se.textContent=`處理中 ${m+1}/${e.length}…`;try{const f=await d.handle.getFile(),u=await new Promise((g,p)=>{const v=new FileReader;v.onload=()=>g(v.result),v.onerror=p,v.readAsDataURL(f)});t[d.name]=u}catch{}}se.disabled=!1,se.textContent="匯出分享版";const n=m=>JSON.stringify(m).replace(/</g,"\\u003c").replace(/>/g,"\\u003e").replace(/&/g,"\\u0026"),r=zn.replace("{{DATA_JSON}}",()=>n({comments:P,characters:O,roleColors:Z})).replace("{{AUDIO_JSON}}",()=>n(t)),s=new Blob([r],{type:"text/html;charset=utf-8"}),o=URL.createObjectURL(s),l=new Date,c=m=>String(m).padStart(2,"0"),a=`${l.getFullYear()}${c(l.getMonth()+1)}${c(l.getDate())}-${c(l.getHours())}${c(l.getMinutes())}`,h=document.createElement("a");h.href=o,h.download=`voicepicker-external-${a}.html`,h.click(),URL.revokeObjectURL(o)}se.addEventListener("click",()=>{se.blur(),mi()});St.addEventListener("click",()=>{St.blur(),G("role")});Hn.textContent="v0.9.7";te||Xe(null);
